...
class UsersDataTable extends DataTable
{
    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ...
            ->editColumn('user', function (Admin $admin) {
                return view('pages/apps.user-management.users.columns._user', compact('admin'));
            })
            ->editColumn('role', function (Admin $admin) {
                return ucwords($admin->roles->first()?->name);
            })
            ->editColumn('last_login_at', function (Admin $admin) {
                return sprintf('<div class="badge badge-light fw-bold">%s</div>', $admin->last_login_at ? $admin->last_login_at->diffForHumans() : $admin->updated_at->diffForHumans());
            })
